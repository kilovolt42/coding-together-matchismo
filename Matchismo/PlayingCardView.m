...
- (void)drawRect:(CGRect)rect {
	...
	if (self.isFaceUp) {
		UIImage *faceImage = [UIImage imageNamed:[NSString stringWithFormat:@"%@-%@.jpg", [self rankAsString], [self suitAsReadableString]]];
		if (faceImage) {
...
- (NSString *)suitAsReadableString {
	if ([self.suit isEqualToString:@"♥"]) return @"hearts";
	if ([self.suit isEqualToString:@"♦"]) return @"diamonds";
	if ([self.suit isEqualToString:@"♠"]) return @"spades";
	if ([self.suit isEqualToString:@"♣"]) return @"clubs";
	return nil;
}
